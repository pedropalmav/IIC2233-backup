...
def filtrar_prohibidos(iterar_peliculas, actor_prohibido):
    ...
    peliculas_filtradas = filter(lambda x: actor_prohibido not in x.actores, iterar_peliculas)
...
def calcular_afinidades(catalogo_peliculas, usuario: Usuario):
    ...
    """
    # Debes completar esta función
    afinidades = map(lambda x: (x, usuario.calificar_pelicula(x)), catalogo_peliculas)
    return afinidades
...
def encontrar_peliculas_comunes(usuarios_watch_party):
    ...
    """
    # Debes completar esta función
    users = usuarios_watch_party
    peliculas_favoritas = reduce(lambda x, y: x.peliculas_favoritas & y.peliculas_favoritas, users)
    peliculas_favoritas = set(peliculas_favoritas)
...
def encontrar_usuario_mas_afin(usuario, otros_usuarios):
    ...
    """
    # Debes completar esta función
    same_actor = filter(lambda x: x.actor_prohibido == usuario.actor_prohibido, otros_usuarios)
    compatibilidad = map(lambda x: (x + usuario, x), same_actor)
    usuario_afin  = reduce(lambda x, y: x[1] if x[0] > y[0] else y[1], compatibilidad)
    return usuario_afin