...
class Usuario:
    ...
    def ver_videos(self, nombre_lista):
        ...
        lista_reproduccion = self.listas_reproduccion[nombre_lista]
        print(lista_reproduccion)
        for pelicula in lista_reproduccion:
            pelicula[0].reproducir()
            calificacion = self.calificar_pelicula(pelicula[0])
            yield (calificacion, pelicula[0].rankings)
