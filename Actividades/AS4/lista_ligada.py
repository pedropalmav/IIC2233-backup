...
class NodoAmigoSecreto:
    ...
    def insertar_amigo_secreto(self, nuevo_nodo, posicion,
        ...
        """
        # Completar
        if posicion == 0 and posicion_actual == 0:
            nuevo = NodoAmigoSecreto(nuevo_nodo, self)
            encontrado = False
            anterior = self.siguiente
            while not encontrado:
                if anterior.siguiente == self:
                    anterior.siguiente = nuevo
                    encontrado = True
                anterior = anterior.siguiente
            return 
        
        elif posicion_actual == posicion - 1:
            nuevo = NodoAmigoSecreto(nuevo_nodo, self.siguiente)
